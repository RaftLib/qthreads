...
typedef struct {
    ...
    aligned_t      my_dc;
    aligned_t      expect;
    aligned_t     *num_descendants;
    void          *p; // actual parent
    void          *d; // descendant node allocation
    aligned_t      nd[MAXNUMCHILDREN];
} node_t;
...
static aligned_t visit_bottom(void *args_);
// Notes:
...
static aligned_t visit(void *args_)
{
    ...
    aligned_t *num_descendants = parent->num_descendants;

    assert(num_children >= 0);
    if (num_children != 0) {
	node_t   *children;

	parent->d = children = malloc(sizeof(node_t) * num_children);
	assert(children);

	if (parent_height == 0 && num_children >= MAXNUMCHILDREN) {
	    num_descendants = parent->num_descendants = malloc(sizeof(aligned_t)*num_children);
	} else {
	    assert(num_children < MAXNUMCHILDREN);
	}

        // Spawn children, if any
        qthread_empty(&parent->my_dc);
	parent->my_dc = 0;
        for (int i = 0; i < num_children; i++) {
	    children[i].height = parent_height + 1;
	    children[i].dc     = &parent->my_dc;
	    children[i].expect = num_children;
	    children[i].p = parent;
	    children[i].num_descendants = (aligned_t*)&(children[i].nd);
            children[i].acc = &num_descendants[i];
            for (int j = 0; j < num_samples; j++) {
                rng_spawn(parent->state.state, children[i].state.state, i);
            }

            children[i].num_children    = calc_num_children(&children[i]);
	    assert(children[i].num_children >= 0);
    ...
    *parent->acc = 1;
    {
	aligned_t tmp = qthread_incr(parent->dc, 1);
	if (tmp + 1 == parent->expect) {
	    if (parent->height == 0) {
...
{
    node_t    *parent          = (node_t *)args_;
    const int  num_children    = parent->num_children;
    aligned_t  sum_descendants = 1;
    aligned_t *num_descendants = parent->num_descendants; // must be in parent
    ...
    *parent->acc = sum_descendants;
    if (qthread_incr(parent->dc, 1) + 1 == parent->expect) {
	if (parent->height == 0) {
	    qthread_fill(parent->dc);
	} else {
	    assert(parent->p);
