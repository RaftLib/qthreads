...
static aligned_t FEB_consumer(void *arg)
{
    ...
    assert(qthread_readFE(&pong, arg) == 0);
    if (pong != 1) {
        iprintf("pong = %u\n", (unsigned)pong);
        assert(pong == 1);
    }
...
static aligned_t FEB_producer(void *arg)
{
    ...
    assert(qthread_writeEF(arg, &ping) == 0);
    return ping;
}
...
static aligned_t FEB_producerloop(void *arg)
{
    ...
    for (i = 0; i < ITERATIONS; i++) {
        ...
        assert(qthread_writeEF(FEBbuffer + offset, &timer) == 0);
        timer = timer ? 0 : 1;
    }
    return 0;
}
...
static aligned_t FEB_consumerloop(void *arg)
{
    ...
    for (i = 0; i < ITERATIONS; i++) {
        assert(qthread_readFE(&timer, FEBbuffer + offset) == 0);
